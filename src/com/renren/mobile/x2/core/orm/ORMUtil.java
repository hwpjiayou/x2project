...
public class ORMUtil {
	...
	public boolean ormUpdate(Class clazz, Object object, ContentValues value) {
		Set<Map.Entry<String, Object>> set = value.valueSet();
	...
	/**
	 * 采用ORM映射必须保证注入ORM注解 {@link com.renren.mobile.x2.core.orm.ORM}
	 * 保证注入的属性是public
	 * */
	...
	public void ormQuery(Class clazz, Object object, Cursor cursor) {
		...
		for (Entry<String, Field> entry : map.entrySet()) {
			Field f = entry.getValue();
			f.setAccessible(true);
			Object o = queryByColumn(entry.getKey(), cursor, entry.getValue()
	...
	/**
	 * @param type
	 *            in{String,Int,Long,Double,Float}
	 * */
