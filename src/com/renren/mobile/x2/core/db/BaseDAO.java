...
public class BaseDAO implements DAO {
	...
	public int querySumCount() {
		...
		Cursor cr = null;
		try {
			cr = database.query(this.mTable.getTableName(),
	...
	public int querySumCount(String where) {
		...
		try {
			cr = database.query(this.mTable.getTableName(),
					new String[] { "count(1)" }, where, null, null, null, null);
			if (cr != null) {
				cr.moveToFirst();
				count = cr.getInt(cr.getColumnIndex("count(1)"));
			}
	 ...
	 *            排序方式 形如 name desc
	 * @param limit
	 *            限制条数 形如 1,2表示从第1条开始的两条(含1条)结果最多为2条
	 * */
	protected final Object query1(Query query, String[] columns,
			String[] whereStrings, String groupBy, String orderBy, String limit) {
		return this.query2(query, columns,
				this.createWhereString(whereStrings), groupBy, orderBy, limit);
	}

	protected final Object query2(Query query, String[] columns,
			String whereString, String groupBy, String orderBy, String limit) {
	...
	public final int getTableRecordCount2(String whereString) {
		int count = (Integer) mQuery_Count.query(
				new String[] { DatabaseColumn.COUNT }, whereString, null, null,
				null);
		return count;
	}
